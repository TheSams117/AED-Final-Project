...
public class Edge<I, ID extends Comparable<ID>, K, V extends Comparable<V>> implements Comparable<Edge<I, ID, K, V>> {
	...
	/**
	 * Compare the edge who arrives by parameter with the current one according to
	 * his weight
	 * 
	 * @param edge Edge to compare
	 * @return Return a integer positive if current edge is greater, a integer
	 *         negative if current edge is less or zero if both edges are equal.
	 */
